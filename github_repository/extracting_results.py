...
def plot_forces_and_moments(file_name):
    ...
    ax1.grid()
    ax1.set_title('{} Forces (Pressure)'.format(case))
    ...
    fig.tight_layout()
    fig.legend()
    plt.savefig('results/{}_forces_plot.png'.format(case))
    plt.close()
...
def average_forces(file_name):
    ...
    if len(fpx) != 2001:
        print(file_name, " is not 2000 iterations long. It is actually ", (len(fpx)-1))
...
def plot_residuals(case):
    ...
    plt.ylabel('Value')
    plt.yscale('log')
    plt.title('{} Residuals'.format(case))
    plt.grid(True)
    plt.legend()

    plt.savefig('results/{}_residuals_plot.png'.format(case))
    plt.close()
...
for directory in directories[:]:
    if directory == 'copy_case':
        directories.remove(directory)
    elif not directory[0].isdigit():
        directories.remove(directory)
